...
Several attempts have been made to create a complete set of templates for categorizing the spectral type of galaxies using data from nearby galaxies~(e.g.\citealt{Kinney93}, \citealt{Kinney96}, hereafter \citealias{Kinney96}, \citealt{Bershady00}, \citealt{Mannucci01}). 
...
%What \citealias{Hossein12} did
~\citet[][hereafter \citealias{Hossein12}]{Hossein12} classified SEDs of a sample of 142 galaxies using a supervised neural network method, based on the spectral template presented by \citealias{Kinney96}.
With the supervised method, they could only classify SED of 105 out of the 142 galaxies.
The SED of the 37 galaxies from their samples could not be matched with any of those in the \citealias{Kinney96} templates. 
In order to classify the remaining 37 galaxies, they combined spectrum of the \citealias{Kinney96} templates.
%This procedure showed that, not a single type of galaxies in \citealias{Kinney96} template could describe the SED of any of those 37 galaxies. %PB20160603: repeats previous sentence
\citealias{Hossein12} also showed that there are tight correlations between physical properties of galaxies, and these correlations might be different for each type of galaxy.

%What we did
In this paper, we compare supervised and unsupervised methods of galaxy spectral classification directly, using exactly the same data as \citealias{Hossein12}.  
...
First, we train self-organizing maps using the \citealias{Kinney96} spectral templates and compare our classification with the \citealias{Kinney96} classification.
Then, we use the trained network to classify the SEDs of 142 galaxies with $0.5 < z < 1$ from \citealias{Hossein12}. 
As in \citealias{Hossein12}, we examine the averaged properties of the new galaxy groups and compare them with previous work.
